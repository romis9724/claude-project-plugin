...
AskUserQuestion으로 아래 4개 질문을 수집한다. 이 답변은 이후 모든 파일 생성에 사용된다.

**Q1. 프로젝트 기본 정보**
- 프로젝트 한글명
- 프로젝트 영문명 (소문자-하이픈, 예: `my-ai-service`)
- 프로젝트 목적 (2~3문장)
- 예상 기간 (예: 2025-01 ~ 2025-06)

**Q2. 기술 스택**
- 시스템 유형: `API서버` / `웹앱(풀스택)` / `데이터파이프라인` / `마이크로서비스` / `AI에이전트시스템`
- 주요 언어 + 프레임워크 (예: `Python + FastAPI`, `Java + Spring Boot`, `Node.js + NestJS`)
- DB 기술 (예: `PostgreSQL`, `MySQL`, `MongoDB`, `없음`)
- 클라우드/인프라 (예: `AWS`, `GCP`, `Azure`, `온프레미스`)

**Q3. 팀·프로세스·하네스**
- 팀 구성 (예: `PM 1, 아키텍트 1, 개발자 3, DBA 1, 보안 1`)
- 스프린트 기간: `1주` / `2주` / `3주` / `4주`
- 하네스 도구: `GitHub Actions` / `GitLab CI` / `Jenkins` / `없음`

**Q4. AI·보안 요구사항**
- LLM 사용: `Claude` / `GPT-4` / `Gemini` / `사용 안 함`
- 외부 API 연동 여부 (있으면 목록)
- 핵심 보안 요건 (예: `개인정보처리`, `금융데이터`, `내부망 전용`, `일반`)
- 인증 방식: `JWT` / `OAuth2` / `API Key` / `SAML` / `없음`

수집된 답변을 변수로 기억: KR_NAME, EN_NAME, PURPOSE, PERIOD, SYSTEM_TYPE, STACK, DB, INFRA, TEAM, SPRINT, HARNESS, LLM, EXT_API, SECURITY_REQ, AUTH
...
- **개인정보**: {SECURITY_REQ 기반 PII 처리 방침}
- **외부 입력**: 항상 검증·이스케이프 후 처리
- **외부 API**: {EXT_API 목록 또는 "없음"}
